...
- **可演化的内容块**：Agent 可在运行时定义新的块类型和渲染方式（参见 [`feature-evolvable-blocks.md`](feature-evolvable-blocks.md)，暂缓）
...
前端负责检测 `text_delta` 流中的 `mutagent:xxx` fenced code block，通过 BlockRenderer 路由到对应的内置渲染器：
...
更多内置块类型（tasks、status、ask、confirm 等）将按需添加。

> 可演化块系统（声明式块引擎、自定义块沙箱）及多媒体内容的完整设计参见 [`feature-evolvable-blocks.md`](feature-evolvable-blocks.md)。
...
- [x] **Task 5.1**: 构建与打包
  - [x] Vite 生产构建 → `src/mutbot/web/frontend_dist/`
  - [x] pyproject.toml package-data（`mutbot.web` 包含 `frontend_dist/**`）
  - [x] `python -m mutbot` 入口（默认启动 Web 服务器）
  - 状态：✅ 已完成 (2026-02-25)
