...
class AuthSetupView(View):
    step: str  # "token_input" | "configure" | "select_provider" | "already_configured"
    error: str
    relay_url: str
...
2. 构造 OAuth URL
3. 通过 `await self.send_command("mutgui.redirect", url=oauth_url)` 让前端整页跳转

随着 mutgui command channel 落地，setup 不再需要伪组件包装副作用；redirect 直接走协议级 command，下发一次且不会因重连/重渲染重放。
...
`setup.js` 由 mutbot frontend vite 构建产出（详见下文「集成方式」与「目录结构」），内部完成：import mutgui core → 注册 antd 组件与 `mutgui.redirect` command → 调用 `MutguiView` 挂载到 `#app` → 建立 WebSocket 到 `/auth/setup/ws`。
...
│   └── main.tsx               # 现有 entry
├── setup/                     # 新增
│   └── index.tsx              # entry: 注册 antd + command handler + mount mutgui
...
### 前端命令处理 — `mutgui.redirect`

setup 页不再注册 `mutbot.Redirect` 自定义组件，直接消费 mutgui command message：

```tsx
import { MutguiView, registerCommands, resolveCommand } from '@mutgui/core';
...
WebSocket message handler 同时处理 `render` 和 `command`；收到 command 后按 `name` 解析并执行，未知命令 warning 后丢弃。
...
- **Q2 前端副作用承载方式** → 不再新建 setup 局部 Redirect 组件，直接复用 mutgui command channel
...
- [x] 新建 `mutbot/frontend/setup/index.tsx`：注册 antd + command handler → mount mutgui → 连 `/auth/setup/ws`
...
- [x] 实现 `step in {token_input, configure, select_provider, already_configured}` 的 antd 控件树
