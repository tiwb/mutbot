...
- [ ] **Task 1.1**: 项目基础设施
  - [ ] 修正 `src/mutbot/` 包结构，创建 `web/` 子包
  - [ ] `pyproject.toml` 添加 mutagent、fastapi、uvicorn 依赖
  - [ ] FastAPI 应用骨架 + 静态文件挂载
  - [ ] `__main__.py` 入口（启动 uvicorn，支持 --host / --port 参数）
  - 状态：⏸️ 待开始

- [ ] **Task 1.2**: Agent 桥接层（sync/async 桥）
  - [ ] `WebUserIO(UserIO)` 子类：`input_stream()`（Queue 阻塞读取）+ `present()`（非 LLM 输出转发）
  - [ ] `asyncio.to_thread` + 双队列桥接（`queue.Queue` 输入 + `asyncio.Queue` 输出）
  - [ ] StreamEvent → JSON 序列化（`serializers.py`）
  - 状态：⏸️ 待开始

- [ ] **Task 1.3**: Workspace + Session 管理基础
  - [ ] `workspace.py`：创建/获取/列出，首次使用自动创建默认 Workspace
  - [ ] `session.py`：创建/获取/列出，Agent 组装（`create_agent`），Session ↔ Agent 生命周期
  - [ ] WebSocket 连接管理 + 广播
  - 状态：⏸️ 待开始

- [ ] **Task 1.4**: 前端骨架
  - [ ] 初始化 Vite + React 19 + TypeScript
  - [ ] ReconnectingWebSocket 客户端
  - [ ] 工作区列表/选择界面
  - [ ] Session 列表面板（侧边栏，点击切换 Session）
  - [ ] 基础 Agent 对话面板（消息列表 + 输入组件）
  - [ ] 端到端验证
  - 状态：⏸️ 待开始

### 阶段二：对话体验增强 [待开始]

- [ ] **Task 2.1**: 流式 Markdown + 代码高亮
  - [ ] react-markdown + Shiki
  - [ ] 内置块类型 → React 组件路由
  - 状态：⏸️ 待开始

- [ ] **Task 2.2**: 工具调用可视化 + 交互块
  - [ ] ToolCallCard（展开/折叠、参数、结果、耗时）
  - [ ] AskBlock / ConfirmBlock → WebSocket 响应
  - 状态：⏸️ 待开始
...
- [ ] **Task 4.1**: Workspace + Session 持久化
  - [ ] 对话历史存储（Message + StreamEvent）
  - [ ] Workspace 布局状态持久化
  - [ ] 掉线重连自动恢复
  - 状态：⏸️ 待开始
